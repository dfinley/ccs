...
export type CodexModelTuningEffort = 'minimal' | 'low' | 'medium' | 'high' | 'xhigh' | 'max';
...
export function normalizeCodexLegacyModelAliases(model: string): string {
  const trimmed = trimModelId(model);
  const { baseModel, suffix } = splitBaseModelAndSuffix(trimmed);
  const exactReplacement = CODEX_LEGACY_MODEL_ALIASES[baseModel.trim().toLowerCase()];
  if (exactReplacement) {
    return `${exactReplacement}${suffix}`;
  }
